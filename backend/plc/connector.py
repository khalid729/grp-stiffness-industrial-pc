...
class PLCConnector:
    ...
    # ══════════════════════════════════════════════════════════════════════
    # OPTIMIZED BLOCK READ - Read entire DB at once for speed
    # ══════════════════════════════════════════════════════════════════════

    def read_db_block(self, db_number: int, start: int, size: int) -> Optional[bytearray]:
        """Read a block of data from DB in one operation
        ...
        Returns:
            bytearray of data or None if error
        """
        if not self.connected:
            return None
        try:
            ...
            self._handle_connection_error(e)
            logger.error(f"Error reading block from DB{db_number}: {e}")
            return None

    def write_db_block(self, db_number: int, start: int, data: bytearray) -> bool:
        """Write a block of data to DB in one operation
        ...
        try:
            ...
            self._handle_connection_error(e)
            logger.error(f"Error writing block to DB{db_number}: {e}")
            return False
